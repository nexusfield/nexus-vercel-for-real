...
async function processSingleChunk(rawText) {
  ...
  // Step 2: Get embedding via Ollama (nomic-embed-text, 768 dimensions)
  ...
  return { module: parsed.module, tags: parsed.tags ?? [] };
}
...
async function runIntakeAgent(rawText) {
  ...
  if (wordCount < WORD_COUNT_THRESHOLD) {
    ...
    const tagsStr =
      Array.isArray(result.tags) && result.tags.length > 0
        ? result.tags.join(", ")
        : "untagged";
    return `Stored as ${result.module} — tagged ${tagsStr}`;
  }
  ...
  return `Split into ${results.length} records — stored as ${modules} tagged ${tagsStr}`;
}
